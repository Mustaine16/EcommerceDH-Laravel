...
const confirmarPasswordError = document.querySelector('#password-confirm + span.error');
...
campoUsuario.onblur = function(event){
  ...
  if(this.value.trim()=="" || this.value.trim().length < nombreMinLength){
    ...
    errores=true;
     ...
     //mostrar mensaje de error
     usuarioError.innerHTML = 'El usuario es muy corto, al menos '+nombreMinLength+ ' caracteres';
     usuarioError.className = 'text-danger';
     errores=true;
   }
  }
...
campoConfirmarPassword.onblur = function(event){
  ...
  if(this.value.trim()=="" || this.value.trim().length < passwordMinLength){
    ...
    errores=true;
     ...
     console.log('El nombre es muy corto pa');
     //mostrar mensaje de error
     confirmarPasswordError.innerHTML = 'El password es muy corto, al menos '+passwordMinLength+' caracteres';
     confirmarPasswordError.className = 'text-danger';
     errores=true;
   }
  }
...
console.log('adentro del onsubmit');
    ...
    if(
      ...
      campoUsuario.value.trim()=="" || campoUsuario.value.trim().length < nombreMinLength||
      campoPassword.value.trim()=="" || campoPassword.value.trim().length < passwordMinLength ||
      campoConfirmarPassword.value.trim()=="" || campoConfirmarPassword.value.trim().length < passwordMinLength ||
      campoEmail.value.trim()=="" || !regexMail.test(campoEmail.value)
